...
// Extended type for rankings with additional columns (until types are regenerated)
type ExtendedRanking = Ranking & {
...
export default function CompareRankingsPage() {
  ...
  const [myRankings, setMyRankings] = useState<ExtendedRanking[]>([])
  const [selectedMyRanking, setSelectedMyRanking] =
    useState<ExtendedRanking | null>(null)

  const [friends, setFriends] = useState<FriendProfile[]>([])
  const [friendRankings, setFriendRankings] = useState<ExtendedRanking[]>([])
  const [selectedFriendRanking, setSelectedFriendRanking] =
  ...
  const [friendFull, setFriendFull] = useState<FullRankingWithItems | null>(
    null
  )
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (!error) setMyRankings(data as ExtendedRanking[])
      setLoadingUser(false)
    }
    init()
  ...
  useEffect(() => {
    ...
    const loadFriendsAndRankings = async () => {
      ...
      if (selectedMyRanking.source_type && selectedMyRanking.source_id) {
        const { data: frRanks } = await supabase
          ...
          .order('created_at', { ascending: false })
        setFriendRankings(frRanks as ExtendedRanking[])
      } else {
        setFriendRankings([])
      }
    ...
    <main className="min-h-screen bg-white py-8 px-4">
        ...
        <div>
          ...
          {!selectedMyRanking ? (
            <p className="text-sm text-gray-600">
              Select one of your fixed rankings first.
            </p>
          ) : loadingFriends || loadingRankings ? (
            <p>Loading friends...</p>
          ) : friendRankings.length === 0 ? (
            <p className="text-sm text-gray-600">
              No friends have a matching ranking yet.
            </p>
          ) : (
            friendRankings.map((r) =>
              renderFriendRankingCard(
                r,
                () => setSelectedFriendRanking(r),
                selectedFriendRanking?.id === r.id
              )
            )
          )}
        </div>
      ...
      {selectedMyRanking && selectedFriendRanking && myFull && friendFull && (
        <div className="max-w-6xl mx-auto mt-10 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="border p-4 rounded-lg bg-gray-50">
            <RankingView
              ranking={friendFull.ranking}
              items={friendFull.items}
            />
          </div>
        </div>
      )}
